...
def filter_events():
    ...
    if popular and popular.lower() == "true":
        if popular_threshold and popular_threshold.lower() == "true":
          filter_by_popular(popular_threshold)
        else:
          filter_by_popular()
...
def filter_by_time(time_period):
...
def filter_by_popular(threshold=None):
  if threshold:
